...
S_prime = S;
[m_s, n_s] = size(S);     % Size of the sparse matrix
K = 6; batch_size = n_s/K;
P = diag(sum(S,2)/n_s)*ones(m_s, n_s); P_prime2 = zeros(m_s, n_s);
...
for i = 1:K
    ...
    P_prime(:,tau(1:min(size(tau,2),batch_size)))=[];
end
% The corresponding result matrix
r_mat(find(S_prime * probe_mat)) = 1;
for i = 1:K
    P = update_probability(probe_mat(:,i), P, r_mat(:,i));
end
for i = 1:n_p
         for j = 1:n_s
